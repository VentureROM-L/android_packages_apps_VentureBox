...
package com.chintans.venturebox.updater;
...
import com.chintans.venturebox.util.Version;
...
public abstract class Updater implements Response.Listener<JSONObject>, Response.ErrorListener {
    ...
    public static final String PROPERTY_MAINTAINER = "ro.venture.maintainer";
    ...
    public boolean isUpdateAvailable(JSONObject jsonObject) {
        final ArrayList<UpdateInfo> updates = new ArrayList<>();
        boolean updateAvailable = false;
        try {
            // get updates
            JSONArray jArray = jsonObject.getJSONArray("updates");
            for (int i = 0; i < jArray.length(); i++) {
                JSONObject jObject = jArray.getJSONObject(i);
                UpdateInfo updateInfo = new UpdateInfo();
                updateInfo.mName = jObject.getString("name");
                updateInfo.mVersion = jObject.getString("version");
                updateInfo.mSize = jObject.getString("size");
                updateInfo.mURL = jObject.getString("url");
                updateInfo.mMD5 = jObject.getString("md5");
                updates.add(updateInfo);
            }
            UpdateInfo updateInfo = updates.get(updates.size() - 1);
            String updateName = updateInfo.mName.replace(".zip", "");
            String deviceName = Utils.getProp("ro.venture.version");
            Log.e("Stuff", updateName + " " + deviceName);
            if (updateName.equals(deviceName)) {
                updateAvailable = false;
            } else {
                updateAvailable = true;
                Log.e("it got here", "it heres");
                return true;
            }
        } catch (JSONException e) {
            e.printStackTrace();
            Log.e("Updater.java", "Something weird happened here dude..");
        }
